...
def _client_frame(payload: bytes, opcode: int = 0x1) -> bytes:
    """Build one FIN+masked RFC6455 client frame."""
...
def _session_ws_send(core: Any, token: str, text: str, channel: str, timeout: float = 6.0) -> dict[str, Any]:
    ...
    # The broadcast can be missed if the peer closes immediately after commit.
    # Verify the durable REST transcript before declaring failure.
...
def apply_core(core: Any) -> None:
    ...
    def send_message(text: str, channel: str = core.DEFAULT_CHANNEL) -> Any:
        ...
        # A registered-agent bearer represents an agent identity and belongs on
        # the agent REST endpoint. kaigi never mints or impersonates that identity.
        if source == "bearer-env":
            return legacy_send(text, channel)

        # Backward compatibility for AgentChattr versions where browser/session
        # auth was accepted directly by /api/send.
